...
namespace esphome
{
    namespace tx_ultimate_touch
    {
        ...
        void TxUltimateTouch::send_touch_(TouchPoint tp)
        {
            ...
            case TOUCH_STATE_SWIPE_1:
                ESP_LOGD(TAG, "Swipe Left (x=%d)", tp.x);
                this->swipe_trigger_1_.trigger(tp);
                break;

            case TOUCH_STATE_SWIPE_2:
                ESP_LOGD(TAG, "Swipe Right (x=%d)", tp.x);
                this->swipe_trigger_2_.trigger(tp);
                break;
        ...
        bool TxUltimateTouch::is_valid_data(int bytes[])
        {
            ...
            if (state != TOUCH_STATE_PRESS &&
                ...
                state != TOUCH_STATE_SWIPE_1 &&
                state != TOUCH_STATE_SWIPE_2 &&
        ...
        int TxUltimateTouch::get_touch_state(int bytes[])
        {
            ...
            {
                if (bytes[5] == TOUCH_STATE_SWIPE_2)
                {
                    state = TOUCH_STATE_SWIPE_2;
                }
                else if (bytes[5] == TOUCH_STATE_SWIPE_1)
                {
                    state = TOUCH_STATE_SWIPE_1;
                }
            }
